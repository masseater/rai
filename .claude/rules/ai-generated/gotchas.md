...
- `rai issue develop` は `tmux new-session` の 750 ms 後にセッション生存を確認し、
  即死していれば `/tmp/rai-issue-develop/<session>.log` の末尾を出して非ゼロ終了する
...
- `FinalizeCmd` は `--engine-cmd` と `--permission-mode` を `build_finalize_command`
  経由で受け取る。新しい engine 関連フラグを `Cmd` に追加するときは、
  `build_finalize_command` と `FinalizeCmd` にも忘れず通すこと。

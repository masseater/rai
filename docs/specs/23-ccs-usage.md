...
- 表示は profile ごとに 1 行で、以下の項目を含む:
  ...
  - 7d 枠の使用率と reset 時刻
  - NOTE 列 (異常時の理由: `no credentials` / `expired` / `auth failed` / `timeout` /
    `no usage yet` 等)
...
- **アクセストークン (`claudeAiOauth.accessToken`) を一切ログ・stdout・stderr に
  ...
  含まれないようにラップする。
- 読み取り専用。ccs の credentials ファイルや keychain は書き換えない。
  expired token を見つけても rai 側で refresh は行わず、ユーザーに `ccs` で再認証
  するよう案内するに留める。
...
- accessToken の自動 refresh (ccs / claude 側の責務)。
